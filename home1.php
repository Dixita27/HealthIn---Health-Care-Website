...
<body>
   ...
   <button type="submit" onclick="getData()" name="search">Search</button>
   <button type="submit" name="search">Search</button> -->

        <form id="searchForm" onsubmit="event.preventDefault(); redirectToResults();">
            <h1>Disease Search</h1>
            <input type="text" id="searchInput" name="searchInput" placeholder="Enter disease name">
...
searchInput.addEventListener('input', function() {
    ...
    if (searchTerm.length > 0) {
        ...
        fetchSuggestions()
            .then(suggestions => {
                // Filter suggestions based on input letters
...
function fetchSuggestions() {
    // Replace 'API_KEY' with your actual API key and update the API endpoint accordingly
    const apiKey = '9f41a40c13df4aeeb293366233c59ec4';
    const apiUrl = `https://api.nhs.uk/conditions/${searchQuery}?subscription-key=${subscriptionKey}`;
                ...
                if (Array.isArray(part.hasPart)) {
                    part.hasPart.forEach(nestedPart => {
                        ...
                        if (nestedPart['@type'] === 'ImageObject' && nestedPart.url) {
                            hasPartHtml += `<p>${nestedPart.name}</p><img src="${nestedPart.url}" alt="Image">`;
                        }
                    });
                }
            });
